...
namespace Unused
{
	...
	public class TerrainObjectIds
	{
		...
		public const int BORDER_TILE = -2;
		...
		void GenerateMap()
		{
			...
			for (int x = 0; x < size; x ++)
			{
				for (int y = 0; y < size; y ++)
				{
					...
					// if terrainId is between 1 and 13, the tile is an edge or corner piece
					if (1 <= terrainId && terrainId <= 12)
					{
					...
					{
						if (terrainId == 0)
							terrainSpriteMap [y, x].sprite = terrainSprites [1];
		...
		{
			//id--;			// reference the MapGenerator class, SumNeighbors() method for why I did this
			int returnId;
			if (id % 3 == 0)
